...
void loop() {
  ...
  if(btn_right_check && btn_left_check != 0 && rtc.lostPower() == true){
    if (currentTime - previousTime_6 >= eventTime_6_write_EEPROM){ 
      ...
      for(int i = 0; i <= day_count; i++){
        eeprom.eeprom_write(i + i + 29, egg_count_record_day[i]);
  ...
  if (rtc.lostPower() == false){
    if (currentTime - previousTime_6 >= eventTime_6_write_EEPROM){ 
     ...
     for(int i = 0; i <= day_count; i++){
        eeprom.eeprom_read(i + i + 29, &egg_count_record_day[i]);
        Serial.print(egg_count_record_day[i]);
      };
     previousTime_6 = currentTime;
